...
MyTransData * parse_file(char * str)
{
	...
	else
	{
		...
		while(str[i] != '\0')
		{
			...
			{
				case Type: // read type
					data = (MyTransData *)malloc(sizeof(MyTransData));
					if(!data)
...
void insert_list(MyList * list, MyTransData * data)
{
	if(MyListEmpty(list))
		(void)MyListAppend(list, (void*)data);
	else
	{
		MyListElem * elem = MyListFirst(list);
		while(elem)
		{
			MyTransData * temp = (MyTransData *)(elem -> obj);
			...
			{
				(void)MyListInsertBefore(list, (void*)data, elem);
				break;
			}
			else
...
void print_list(MyList *list)
{
    ...
    int amount_value = 0;
    for (elem = MyListFirst(list); elem != NULL; elem = MyListNext(list, elem))
    {
        MyTransData * data = (MyTransData *)(elem -> obj);
